...
def get_players(a: int , b: int ):
    conn = get_connection()
    try:
        query = "SELECT DISTINCT name, p.player_id FROM players p INNER JOIN match_events e ON p.player_id = e.player_id WHERE match_id = %s and e.team_id = %s and e.period_display_name = 'FirstHalf' ORDER BY name"
        df = pd.read_sql(query, conn, params=(int(a), int(b)))
        return df
    finally:
        conn.close()
